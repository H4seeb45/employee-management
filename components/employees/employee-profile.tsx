            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  <CustomTag color={getDepartmentColor(employee.department)}>
                    {employee.department}
                  </CustomTag>
                  <CustomTag color={getStatusColor(employee.status)}>
                    {employee.status}
                  </CustomTag>
                </div>
              </div>
            ...
            <Card title="Recent Reviews" className="shadow-sm">
              <Timeline>
                {employee.reviews?.map((review: any, index: number) => (
                  <Timeline.Item
                    key={index}
                    color={getReviewColor(review.rating)}
                  >
              ...
              <Timeline>
                {employee.achievements?.map(
                  (achievement: any, index: number) => (
                    <Timeline.Item
                      key={index}
                      dot={<Award className="h-4 w-4 text-amber-500" />}
                    >
