  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/payroll/adjustments?month=${month}&year=${year}${locationQuery}`,
      );
      if (res.ok) {
  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (res.ok) {
          toast.success(
            `Successfully uploaded ${formattedRecords.length} records`,
          );
          fetchRecords();
        } else {
                ...
                filteredRecords.map((r) => (
                  <TableRow key={r.id} className="text-xs sm:text-sm">
                    ...
                    <TableCell className="text-right font-medium">
                      {formatCurrency(r.attendanceAllowance)}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {formatCurrency(r.dailyAllowance)}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                    ...
                    <TableCell className="text-right font-medium">
                      {formatCurrency(r.maintainence)}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {formatCurrency(r.eachKpiIncentives)}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {formatCurrency(r.categoryIncentive)}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {formatCurrency(r.olpersMilk)}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {formatCurrency(r.olpersCareem)}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {formatCurrency(r.eidIncentive)}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {formatCurrency(r.olpers500ml)}
                    </TableCell>
                    <TableCell className="text-right text-green-600 font-medium">
                      {formatCurrency(r.incentives)}
                    </TableCell>
                    <TableCell className="text-right text-green-600 font-medium">
                      {formatCurrency(r.comission)}
                    </TableCell>
                    <TableCell className="text-right text-red-600 font-medium">
                      {formatCurrency(r.shortages)}
                    </TableCell>
                    <TableCell className="text-right text-red-600 font-medium">
                      {formatCurrency(r.marketCredit)}
                    </TableCell>
                  </TableRow>
                ))
              )}
