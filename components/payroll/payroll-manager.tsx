...
export function PayrollManager({
  ...
  const filteredPayrolls = useMemo(() => {
    return payrolls.filter((p) => {
      const nameMatch = (p.employeeName || p.employee?.employeeName || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
      const idMatch = (p.empId || p.employee?.employeeId || "")
        .toLowerCase()
        .includes(searchId.toLowerCase());
      const designationMatch = (p.designation || p.employee?.position || "")
        .toLowerCase()
  ...
  const fetchPayrolls = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/payroll?month=${month}&year=${year}${locationQuery}`,
      );
      if (res.ok) {
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const empRes = await fetch(
        `/api/employees?status=Active:Working${locationQuery}`,
      );
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        const res = await fetch(
          `/api/payroll/calculate?month=${month}&year=${year}&employeeIds=${chunk.join(",")}${locationQuery}`,
        );
      ...
      setIsSaved(false);
      toast.success(
        "Payroll calculation completed for " + employees.length + " employees",
      );
    } catch (err) {
      console.error(err);
  ...
  const handleExportExcel = () => {
    ...
    const exportData = filteredPayrolls.map((p) => ({
      Location: p.locationName || p.employee?.location?.name || "-",
      "EMP ID": p.empId || p.employee?.employeeId,
      Name: p.employeeName || p.employee?.employeeName,
      ...
      "Att. Allow.": p.attendanceAllowance || 0,
      "Daily Allow.": p.dailyAllowance || 0,
      "Fuel Allow.": p.fuelAllowance || 0,
      "Conv. Allow.": p.conveyanceAllowance || 0,
      Maintenance: p.maintainence || 0,
      Commission: p.comission || 0,
      "KPI Inc.": p.eachKpiIncentives || 0,
      "Cat. Incentive": p.categoryIncentive || 0,
      "Olpers Milk": p.olpersMilk || 0,
      "Olpers Careem": p.olpersCareem || 0,
      "Eid Inc.": p.eidIncentive || 0,
      "Olpers 500ML": p.olpers500ml || 0,
      Incentives: p.incentives || 0,
      "Loaders Allow.": p.loadersAllowance || 0,
      "Total Inc.": p.totalIncentives || 0,
      "Gross Salary": p.grossSalary,
      EOBI: p.eobi || 0,
      "S. Security": p.socialSecurity || 0,
      "I.Tax Ded": p.incomeTax || 0,
      Advance: p.advance || 0,
      Loan: p.loan || 0,
      Shortages: p.shortages || 0,
      "M. Credit": p.marketCredit || 0,
      "Total Ded.": p.totalDeduction,
  ...
  return (
    <div className="space-y-6">
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50/50 dark:bg-slate-800/50">
                  <TableHead className="sticky left-0 bg-slate-50 dark:bg-[#1E293B] z-10 min-w-[100px] border-r dark:border-slate-700">
                    Actions
                  </TableHead>
                  ...
                  <TableHead className="min-w-[120px] font-bold text-[#0A192F] dark:text-white">
                    Net Salary
                  </TableHead>
                </TableRow>
              </TableHeader>
                  ...
                  filteredPayrolls.map((p) => (
                    ...
                    >
                      ...
                      <TableCell className="dark:text-slate-400">
                        {p.locationName || p.employee?.location?.name || "-"}
                      </TableCell>
                      <TableCell className="font-mono text-xs dark:text-slate-300">
                        {p.empId || p.employee?.employeeId}
                      </TableCell>
                      <TableCell className="font-medium dark:text-slate-200">
                        {p.employeeName || p.employee?.employeeName}
                      </TableCell>
                      <TableCell className="dark:text-slate-400">
                        {p.designation || p.employee?.position}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant="outline"
                      ...
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.basicPayable)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.attendanceAllowance)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.dailyAllowance)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.fuelAllowance)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.conveyanceAllowance)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.maintainence)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.comission)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.eachKpiIncentives)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.categoryIncentive)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.olpersMilk)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.olpersCareem)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.eidIncentive)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.olpers500ml)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.incentives)}
                      </TableCell>
                      <TableCell className="dark:text-slate-300">
                        {formatCurrency(p.loadersAllowance)}
                      </TableCell>
                      <TableCell className="font-semibold dark:text-slate-200">
                        {formatCurrency(p.totalIncentives)}
                      </TableCell>
                      <TableCell className="font-bold text-slate-900 dark:text-slate-100">
                        {formatCurrency(p.grossSalary)}
                      </TableCell>
                      <TableCell className="text-red-500 dark:text-red-400">
                        {formatCurrency(p.eobi)}
                      </TableCell>
                      <TableCell className="text-red-500 dark:text-red-400">
                        {formatCurrency(p.socialSecurity)}
                      </TableCell>
                      <TableCell className="text-red-500 dark:text-red-400">
                        {formatCurrency(p.incomeTax)}
                      </TableCell>
                      <TableCell className="text-red-500 dark:text-red-400">
                        {formatCurrency(p.advance)}
                      </TableCell>
                      <TableCell className="text-red-500 dark:text-red-400">
                        {formatCurrency(p.loan)}
                      </TableCell>
                      <TableCell className="text-red-500 dark:text-red-400">
                        {formatCurrency(p.shortages)}
                      </TableCell>
                      <TableCell className="text-red-500 dark:text-red-400">
                        {formatCurrency(p.marketCredit)}
                      </TableCell>
                      <TableCell className="font-semibold text-red-600 dark:text-red-500">
                        {formatCurrency(p.totalDeduction)}
                      </TableCell>
                      <TableCell className="font-bold text-[#0A192F] dark:text-blue-400 bg-slate-50/50 dark:bg-slate-800/50">
      ...
      {viewingSlip && (
        <Dialog
          open={!!viewingSlip}
          onOpenChange={(o) => !o && setViewingSlip(null)}
        >
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Salary Slip Preview</DialogTitle>
              <DialogDescription>
                Viewing salary slip for{" "}
                {viewingSlip.employeeName || viewingSlip.employee?.employeeName}{" "}
            ...
            <DialogFooter className="sticky bottom-0 bg-white dark:bg-[#1E293B] pt-4 border-t dark:border-slate-700">
              <Button
                onClick={() => handlePrint()}
                className="bg-[#0A192F] hover:bg-[#162a45]"
              >
        ...
        <DialogContent className="max-w-md text-center py-12 bg-white dark:bg-[#1E293B] border-none shadow-2xl">
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-bold text-[#0A192F] dark:text-white">
                Calculating Payroll...
              </h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
                Processing salary, deductions, and incentives batches.
              </p>
            </div>
            <Progress
